...
def load_scoring_prompt():
    """
    加载评分门控提示词：指导 LLM 判断检索文档是否足够回答用户问题。
    """
    try:
        # 获取评分提示词文件的绝对路径
        scoring_prompt_path = get_abs_path(prompts_conf["scoring_prompt_path"])
    except KeyError as e:
        logger.error(f"[load_scoring_prompt]在yaml配置项中没有scoring_prompt_path配置项")
        raise e

    try:
        # 打开文件
        # .read()是读取整个文件的内容
        return open(scoring_prompt_path, "r", encoding="utf-8").read()
    except FileNotFoundError as e:
        logger.error(f"[load_scoring_prompt]加载评分提示词出错，{str(e)}")
        raise e


def load_rewrite_prompt():
    """
    加载查询重写提示词：指导 LLM 对用户问题进行 Step-back / HyDE 扩展。
    """
    try:
        # 获取重写提示词文件的绝对路径
        rewrite_prompt_path = get_abs_path(prompts_conf["rewrite_prompt_path"])
    except KeyError as e:
        logger.error(f"[load_rewrite_prompt]在yaml配置项中没有rewrite_prompt_path配置项")
        raise e
    ...
    try:
        ...
        logger.error(f"[load_rewrite_prompt]加载重写提示词出错，{str(e)}")
        raise e
