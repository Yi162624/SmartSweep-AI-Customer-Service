...
- **LangGraph 多阶段 RAG Pipeline** — 「初次检索 → LLM评分门控 →（通过→直接生成）/（不通过→Step-back+HyDE查询重写→二次检索→生成）」的 DAG 流程
- **混合检索 + 重排序** — BM25 关键词（0.7）+ 向量语义检索（0.3）+ FlagReranker 语义重排
- **ReAct Agent 工具调用** — 自主调用 RAG检索、天气查询、地理定位、使用报告生成等工具
- **长期记忆** — MySQL 持久化用户画像与对话历史，实现跨会话个性化问答
